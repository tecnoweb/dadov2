...
var Xcrud = {
		...
		jQuery.ajax({
			...
			success: function(response) {
				...
				if (!response || response.trim() === '') {
					return; // Don't replace content if empty
				}
				
				// Ensure we're updating the correct container by its ID
				var containerId = jQuery(container).attr('id');
				var $targetContainer = containerId ? jQuery('#' + containerId) : jQuery(container);
				
				// Make sure we're only updating our specific container
				if ($targetContainer.length === 1) {
					// Use empty().append() instead of html() as it's more reliable with large/complex content
					try {
						$targetContainer.empty().append(response);
						
						// Verify insertion was successful
						if ($targetContainer.html().length < 100 && response.length > 1000) {
							// Try direct HTML as fallback
							$targetContainer[0].innerHTML = response;
						}
					} catch(e) {
						// Final fallback
						$targetContainer[0].innerHTML = response;
                ...
                var status = Xcrud.check_message($targetContainer[0] || container);
				jQuery(document).trigger("xcrudafterrequest", [$targetContainer[0] || container, data, status]);
				if (success_callback) {
					success_callback($targetContainer[0] || container);
				}
			},
	...
	get_container: function(element) {
		// Find the xcrud-ajax container with the correct instance ID
		var $ajax = jQuery(element).closest(".xcrud-ajax");
		if ($ajax.length && $ajax.attr('id')) {
			// Return the container by its unique ID to ensure we get the right one
			return jQuery('#' + $ajax.attr('id'));
		}
		return $ajax;
	},
	list_data: function(container, element) {
        ...
        if (element) {
            if (jQuery.isPlainObject(element)) {
                ...
                // Handle DOM element - extract data attributes
                var $element = jQuery(element);
                if ($element.data('task')) {
                    data.task = $element.data('task');
                }
                if ($element.data('after')) {
                    data.after = $element.data('after');
                }
                if ($element.data('primary')) {
                    data.primary = $element.data('primary');
                }
                // Copy any other data attributes
                jQuery.each($element.data(), function(key, value) {
                    if (key !== 'task' && key !== 'after' && key !== 'primary') {
                        data[key] = value;
                    }
                });
            }
		...
		// Extract instance name from container ID
		var $container = jQuery(container);
		if ($container.attr('id')) {
			// Extract instance name from ID (format: xcrud-{instance_name})
			var containerId = $container.attr('id');
			if (containerId.indexOf('xcrud-') === 0) {
				data.instance = containerId.substring(6); // Remove 'xcrud-' prefix
			}
		}
		
		// Fallback to data-instance if no ID
		if (!data.instance) {
			var $xcrud = $container.closest('.xcrud');
			if ($xcrud.data('instance')) {
				data.instance = $xcrud.data('instance');
			}
		}
	...
	check_container: function(element, container) {
		var elementContainerId = jQuery(element).closest(".xcrud-ajax").attr('id');
		var targetContainerId = jQuery(container).attr('id');
		// Ensure both have IDs and they match
		return elementContainerId && targetContainerId && elementContainerId === targetContainerId;
	},
...
/*else {
	...
	upload_file: function(element, data, container) {
		...
		jQuery.ajaxFileUpload({
			...
			success: function(out) {
				Xcrud.hide_progress(container);
				// Use safer method for replacing content
				var $container = jQuery(upl_container);
				var $newContent = jQuery(out);
				$container.after($newContent).remove();
	...
	bootstrap_modal: function(header, content) {
		...
		// Use empty().append() for better handling of complex content
		jQuery("#xcrud-modal-window .modal-content").empty().append('<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">' + header + '</h4></div>');
